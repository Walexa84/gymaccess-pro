...
import { 
  ShieldCheck, RefreshCw, ExternalLink, HelpCircle, Layers, 
  DoorOpen, CheckCircle2, ArrowRight, BookOpen, AlertCircle
} from 'lucide-react';
...
export const NivelesAccesoTab: React.FC = () => {
  ...
  const cargarNiveles = async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/iam/niveles-acceso');
      const data = await res.json();
      if (Array.isArray(data)) {
        setNiveles(data);
      }
    } catch (err) {
      console.error('Error cargando niveles de acceso:', err);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
          ...
          <div className={`p-4 rounded-xl border flex flex-col justify-between ${
            ...
            <div>
              ...
              <p className="text-xs text-muted-theme mt-2 leading-relaxed">
                Presiona el botón <strong className="text-main-theme">Sincronizar con Teams</strong> aquí abajo.
              </p>
              <p className="text-xs text-muted-theme mt-1 leading-relaxed">
                El sistema descargará automáticamente los identificadores de nube y vinculará las puertas correspondientes en la base de datos local.
              </p>
            </div>
          </div>
      ...
      <div className="flex items-center justify-between">
        ...
        >
          ...
          <span>{syncing ? 'Sincronizando...' : 'Sincronizar con Teams'}</span>
        </button>
      </div>
            ...
            >
              ...
              <div className="mt-4 pt-3 border-t border-theme/60 flex items-center justify-between text-xs text-muted-theme">
                <span className="flex items-center gap-1">
                  ...
                  <span>Disponible para socios y staff</span>
                </span>
